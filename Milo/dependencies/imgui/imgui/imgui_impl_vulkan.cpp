...
static void CreateOrResizeBuffer(VkBuffer& buffer, VkDeviceMemory& buffer_memory, VkDeviceSize& p_buffer_size, size_t new_size, VkBufferUsageFlagBits usage)
{
	...
	alloc_info.memoryTypeIndex = ImGui_ImplVulkan_MemoryType(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, req.memoryTypeBits);
...
inline void ImGui_ImplVulkan_UpdateTexture(const ImageDescriptor& desc)
{
	...
	writeDescriptorSet.pImageInfo = &desc.image;
	writeDescriptorSet.dstSet = desc.set;
	writeDescriptorSet.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	writeDescriptorSet.descriptorCount = 1;
	writeDescriptorSet.dstBinding = 0;

	vkUpdateDescriptorSets(g_VulkanInitInfo.Device, 1, &writeDescriptorSet, 0, nullptr);
}
...
ImTextureID ImGui_ImplVulkan_AddTexture(uint32_t textureId, VkSampler sampler, VkImageView image_view, VkImageLayout image_layout)
{
	...
	if(s_ImageDescriptors.find(textureId) != s_ImageDescriptors.end()) {
		...
		desc.image.imageView = image_view;
		desc.image.imageLayout = image_layout;
	...
	s_ImageDescriptors[textureId] = desc;
...
void ImGui_ImplVulkan_DeleteTexture(uint32_t textureId)
{
	if(s_ImageDescriptors.find(textureId) == s_ImageDescriptors.end()) return;
	const auto& desc = s_ImageDescriptors[textureId];
	freeDescriptorSets.push(desc.set);
	s_ImageDescriptors.erase(textureId);
}

void ImGui_ImplVulkan_ClearCache()
{
	s_ImageDescriptors.clear();
}

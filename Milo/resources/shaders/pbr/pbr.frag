...
layout(std140, set = 0, binding = 2) uniform SkyboxUniformBuffer {
...
layout(set = 1, binding = 4) uniform sampler2D u_MetallicRoughnessMap;
layout(set = 1, binding = 5) uniform sampler2D u_OcclusionMap;
...
void main() {
    ...
    out_FragColor = vec4((color + emissive).rgb, u_Material.alpha);
}